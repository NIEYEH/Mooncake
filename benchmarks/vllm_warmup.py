...
request.  This helper sends completed inference requests with several prompt
shapes, then keeps a small concurrent load running for a minimum duration.
...
DEFAULT_PROMPT_TOKEN_COUNTS = (8, 128, 512)
...
def parse_prompt_token_counts(value: str) -> tuple[int, ...]:
    try:
        counts = tuple(int(item.strip()) for item in value.split(","))
    except ValueError as exc:
        raise argparse.ArgumentTypeError(
            "prompt token counts must be comma-separated integers"
        ) from exc
    if not counts or any(count <= 0 for count in counts):
        raise argparse.ArgumentTypeError(
            "prompt token counts must contain positive integers"
        )
    return counts
...
def _run_batch(
    ...
    with ThreadPoolExecutor(max_workers=config.concurrency) as pool:
        ...
        for request_id in ids:
            prompt_tokens = config.prompt_token_counts[
                request_id % len(config.prompt_token_counts)
            ]
            ...
            futures[future] = (request_id, prompt_tokens)

        results = []
        for future in as_completed(futures):
            request_id, prompt_tokens = futures[future]
            result = future.result()
            result.update(
                {"request_id": request_id, "approximate_prompt_tokens": prompt_tokens}
            )
...
def run_warmup(config: WarmupConfig) -> dict[str, Any]:
    ...
    # Execute one shape at a time first.  A completed response means the model,
    # scheduler, attention metadata path, and its synchronous Triton launches
    # have all run before any benchmark timer starts.
    for prompt_tokens in config.prompt_token_counts:
        payload = build_payload(
            config.model,
            config.namespace,
            request_id,
    ...
    while (
        ...
        results.extend(_run_batch(config, api_url, batch_ids))
    ...
    return {
        ...
        "compile_probe_requests": len(config.prompt_token_counts),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--prompt-token-counts",
        type=parse_prompt_token_counts,
        default=DEFAULT_PROMPT_TOKEN_COUNTS,
        help="comma-separated approximate prompt lengths (default: 8,128,512)",
    )
    parser.add_argument(
        "--requests",
        type=_non_negative_int,
        default=16,
        help="minimum concurrent-load requests after serial probes (default: %(default)s)",
    )
    parser.add_argument(
        "--concurrency",
        type=_positive_int,
    ...
    parser.add_argument(
        ...
        type=_positive_int,
        default=8,
        help="generated tokens per request (default: %(default)s)",
    )
    parser.add_argument(
