...
TEST_F(MasterServiceSSDTest, PutEndBothReplica) {
    ...
    const std::vector<std::string> batch_keys = {key, "missing_disk_key"};
    auto batch_result =
        service_->BatchGetReplicaList(client_id, batch_keys, "default");
    ASSERT_EQ(batch_result.size(), batch_keys.size());
    ASSERT_TRUE(batch_result[0].has_value());
    EXPECT_EQ(batch_result[0]->replicas.size(), 2u);
    ASSERT_FALSE(batch_result[1].has_value());
    EXPECT_EQ(batch_result[1].error(), ErrorCode::OBJECT_NOT_FOUND);
}
...
TEST_F(MasterServiceSSDTest, PutStartExpires) {
    ...
    auto test_discard_replica = [&](ReplicaType discard_type) {
        ...
        for (size_t i = 0; i <= master_config.put_start_discard_timeout_sec;
            ...
            auto get_result =
                service_->GetReplicaList(client_id, key, "default");
